<template>
	<div class="main-content" :style='{"width":"100%","padding":"30px","margin":"0","overflow-y":"auto","background":"#fff","height":"auto"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('kechengxinxi','选择')" @click="kechengxuanzeCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
		...
		<kechengxuanze-cross-add-or-update v-if="kechengxuanzeCrossAddOrUpdateFlag" :parent="this" ref="kechengxuanzeCrossaddOrUpdate"></kechengxuanze-cross-add-or-update>
...
import kechengxuanzeCrossAddOrUpdate from "../kechengxuanze/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				kechengxuanzeCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			kechengxuanzeCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.kechengxuanzeCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.kechengxuanzeCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.kechengxuanzeCrossaddOrUpdate.init(row.id,type);
				});
			},
