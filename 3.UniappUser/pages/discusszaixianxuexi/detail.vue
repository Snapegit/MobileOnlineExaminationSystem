...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusszaixianxuexiCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusszaixianxuexiCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discusszaixianxuexi', this.id);
		...
		methods: {
			...
			delClick(id){
				...
				uni.showModal({
					...
					async success(res) {
						if(res.confirm){
							await that.$api.del('discussdiscusszaixianxuexi',JSON.stringify([id]))
							that.$utils.msg('删除成功')
							that.detail.discussnum--
							await that.$api.update('discusszaixianxuexi',that.detail)
							setTimeout(()=>{
